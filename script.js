function calcular() {
    ...
    if (pt.value > 10 || pt.value < 0 ) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    } if (st.value > 10 || st.value < 0) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    } if (p.value > 10 ||  p.value < 0) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    }
     else {
 ...
 function verificar() {
    ...
    if (tum.value < 0 || tum.value > 10) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    } if (tdois.value < 0 || tdois.value > 10) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    } if (ttres.value < 0 || ttres.value > 10) {
        window.alert('Erro! Os dados estão incorretos, insira novamente')
    } else {
